...
- (void)handleMessage:(NSData *)data onTopic:(NSString *)topic retained:(BOOL)retained {
    ...
    if([dict[@"type"] isEqualToString:@"add"]){
        ...
        [self addShapeToViewByType:type addX:x addY:y addWidth:width];
        ...
        [self refreshShapeView:index offsetX:x offsetY:y];
    }
...
-(void)setupUI{
    ...
    [clearbutton addTarget:self action:@selector(clearShape) forControlEvents:UIControlEventTouchDown];
...
-(void)addShape{
    ...
    [self addShapeToViewByType:type addX:baseX addY:baseY addWidth:sizeWidth];
}
...
-(void)addShapeToViewByType:(NWShapeViewType)type addX:(CGFloat)x addY:(CGFloat)y addWidth:(CGFloat)width{
    NWDynamicShapeView *view = [[NWDynamicShapeView alloc] initWithFrame:CGRectMake(x, y, width,width)];
    view.delegate = self;
    view.tag = self.shapeNum;
    switch (type){
...
- (void)sendDataWithType:(NSString *)type
    ...
    if([type isEqualToString:@"add"]){
        NSDictionary *dict = @{@"type":type,@"shapetype":[NSNumber numberWithInt:shapeType],@"x":[NSNumber numberWithInt:x],@"y":[NSNumber numberWithInt:y],@"width":[NSNumber numberWithInt:width],@"userid":self.username};
        ...
        data = [type dataUsingEncoding:NSUTF8StringEncoding];
    }else{
        NSDictionary *dict = @{@"type":type,@"x":[NSNumber numberWithInt:x],@"y":[NSNumber numberWithInt:y],@"index":[NSNumber numberWithInt:index],@"userid":self.username};
        data = [NSJSONSerialization dataWithJSONObject:dict options:NSJSONWritingPrettyPrinted error:nil];
    }
